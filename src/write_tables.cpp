...
void write_Total_Xsec_PiRho_PiGamma() {
  ...
  Total_Xsections_PiRho_PiGamma.open("Total_Xsec_PiRho_PiGamma_mrho_" + std::to_string(int(mrho*1000)) + "_MeV.txt");

  Total_Xsections_PiRho_PiGamma << "# sqrt(s), C11, C12, C13, C14, C15, C16"  << std::endl;
  for (int i =1; i < 1002; i +=1){
    double sqrt_s = mpion + mrho + i*0.005;
     ...
     Total_Xsections_PiRho_PiGamma << sqrt_s << "\t" << sigma_C11 << "\t" << sigma_C12 << "\t" << sigma_C13
               << "\t" << sigma_C14 << "\t" << sigma_C15 << "\t" << sigma_C16  << std::endl;
   }
...
void write_Diff_Xsec_PiRho_PiGamma() {
  ...
  Diff_Xsections_PiRho_PiGamma.open("Diff_Xsec_PiRho_PiGamma_mrho_" + std::to_string(int(mrho*1000)) + "_MeV.txt");

  double sqrt_s = 1.0;
	Diff_Xsections_PiRho_PiGamma << "# Differential sigma at sqrt(s) = " + std::to_string(sqrt_s) + " GeV"  << std::endl;
	Diff_Xsections_PiRho_PiGamma << "# t, C11, C12, C13, C14, C15, C16"  << std::endl;
  ...
  double t;
	...
	for (int i =1; i < 1002; i +=1){
		...
		Diff_Xsections_PiRho_PiGamma << t << "\t" << diff_sigma_C11 << "\t" << diff_sigma_C12 << "\t" << diff_sigma_C13
						  << "\t" << diff_sigma_C14 << "\t" << diff_sigma_C15 << "\t" << diff_sigma_C16  << std::endl;
	}
...
void write_Total_Xsec_PiPi_RhoGamma() {
  ...
  Total_Xsections_PiPi_RhoGamma.open("Total_Xsec_PiPi_RhoGamma_mrho_" + std::to_string(int(mrho*1000)) + "_MeV.txt");
  ...
  for (int i =1; i < 1002; i +=1){
    ...
    Total_Xsections_PiPi_RhoGamma << sqrt_s << "\t" << sigma_C21 << "\t" << sigma_C22 << std::endl;
  }
...
void write_Diff_Xsec_PiPi_RhoGamma() {
  ...
  Diff_Xsections_PiPi_RhoGamma.open("Diff_Xsec_PiPi_RhoGamma_mrho_" + std::to_string(int(mrho*1000)) + "_MeV.txt");

  double sqrt_s = 1.0;
	Diff_Xsections_PiPi_RhoGamma << "# Differential sigma at sqrt(s) = " + std::to_string(sqrt_s) + " GeV"  << std::endl;
  ...
  double t;
	...
	for (int i =1; i < 1002; i +=1){
		...
		Diff_Xsections_PiPi_RhoGamma << t << "\t" << diff_sigma_C21 << "\t" << diff_sigma_C22 << "\t" << std::endl;
	}
